...
func SubmitToLogin(w http.ResponseWriter, r *http.Request) {
	...
	employer, err := apiEmployers.CheckEmployerByUsername(username)
	if err != nil {
		logrus.Errorf("Error: %s", err)
		return
	}

	if err := bcrypt.CompareHashAndPassword([]byte(employer.PasswordHash), []byte(password)); err != nil {
		logrus.Errorf("Error: %s", err)
		return
	}

	token, err := api.NewApiEmployers().LoginEmployer()
	if err != nil {
